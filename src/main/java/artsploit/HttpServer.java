...
public class HttpServer implements HttpHandler {
  ...
  public static void start() throws Exception {
    ...
    com.sun.net.httpserver.HttpServer httpServer = com.sun.net.httpserver.HttpServer
        .create(new InetSocketAddress(Config.httpPort), 10);
  ...
  /**
   * Patch the bytecode of supplied class constructor by injecting execution of a
   * command
   */
    ...
    try {
      ...
      switch (path) {
        ...
        case "/xxeLog":
          ...
          // hacky way to access private fields of
          // (Request)((ExchangeImpl)((HttpExchangeImpl)httpExchange).impl).req
        ...
        default:
          if (path.startsWith("/callback")) {
            System.out.println("received calback url " + path);
            printPayload(httpExchange);
  ...
  private void printHeaders(HttpExchange httpExchange) {
    Set<Entry<String, List<String>>> headers = httpExchange.getRequestHeaders().entrySet();
  ...
  private void printPayload(HttpExchange httpExchange) {
